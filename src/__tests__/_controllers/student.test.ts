...
import { mockStudents, newStudent1 } from "../../test_data/user.data";
...
describe("create user or student-specific", () => {
    ...
    it("should create a user when request body is complete", async () => {
        ...
        expect(User.findOne).toHaveBeenCalledWith({ email: newStudent1.email });
        expect(bcrypt.genSalt).toHaveBeenCalled();
        expect(bcrypt.genSalt).toHaveBeenCalledWith(10);
        expect(bcrypt.hash).toHaveBeenCalled();
        expect(bcrypt.hash).toHaveBeenCalledWith(newStudent1.password, "salt");
...
describe("update a student info", () => {
    ...
    it("should validate request body", async () => {
        ...
        await StudentController.updateSingleStudent(mockRequest, mockResponse);
        expect(requestBodyErrorsInterrupt).toHaveBeenCalled();
...
describe("fetch all students", () => {
    ...
    jest.spyOn(Student, "countDocuments").mockReturnValue(2 as any);

    it("should return 200 with an paginated data (student)", async () => {
        await StudentController.getAllStudents(mockRequest, mockResponse);
        expect(Student.find).toHaveBeenCalled();
        expect(Student.find).toHaveBeenCalledWith({}); // empty query {}
        expect(Student.find).toHaveBeenCalledTimes(1);
        expect(Student.countDocuments).toHaveBeenCalled();
        expect(Student.countDocuments).toHaveBeenCalledWith({}); // empty query to signify all students
        expect(mockResponse.status).toHaveBeenCalled();
...
describe("delete a student", () => {
    it("should return 200 when user is deleted", async () => {
        ...
        jest.spyOn(Student, "findByIdAndDelete").mockReturnValue({
            _id: "675ec943a1368b32c795f301",
            level: "100",
            cgpa: "4.0",
            user: {
                name: "John Doe",
